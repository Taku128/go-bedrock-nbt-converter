...
import (
	...
	"github.com/ntaku256/go-bedrock-nbt-converter/data"
)

type chunkerData struct {
	Names   map[string]string                       `json:"names"`
	Flatten map[string]map[string]map[string]string `json:"flatten"`
}

var mappings chunkerData

func init() {
	if err := json.Unmarshal(data.ChunkerMappings, &mappings); err != nil {
...
func MapBlock(bedrockName string, bedrockProps map[string]interface{}) JavaBlockState {
	...
	// 2. Resolve Java name via Chunker mappings
	var javaName string

	// 2a. Conditional flatten rules
	if rules, ok := mappings.Flatten[bedrockName]; ok {
		for stateKey, valueMap := range rules {
			if propVal, has := props[stateKey]; has {
				if resolved, found := valueMap[propVal]; found {
					javaName = resolved
	...
	// 2b. Simple name lookup
	if javaName == "" {
		if name, ok := mappings.Names[bedrockName]; ok {
			javaName = name
		} else {
			// 2c. Fallback
			javaName = "minecraft:" + strings.TrimPrefix(bedrockName, "minecraft:")
		}
	}
